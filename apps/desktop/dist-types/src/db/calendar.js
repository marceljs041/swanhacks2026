 ...
 */
export async function listEventsForRange(fromIso, toIso, opts = {}) {
    const db = await getDb();
    const params = [toIso, fromIso];
    let where = "deleted_at is null and start_at < ? and end_at > ?";
    ...
    return db
        .prepare(`select * from calendar_events where ${where} order by start_at`)
        .all(...params);
}
...
export async function upsertEvent(input, opts = {}) {
    const db = await getDb();
    const ts = nowIso();
    const row = {
    ...
    db.prepare(`insert into calendar_events
     ...
     on conflict(id) do update set
       ...
       sync_version=excluded.sync_version`).run(row);
    if (!opts.skipOutbox)
        await enqueue("calendar_events", row.id, "upsert", row);
    return row;
}
export async function softDeleteEvent(id) {
    const db = await getDb();
    const ts = nowIso();
    db.prepare("update calendar_events set deleted_at = ?, updated_at = ? where id = ?").run(ts, ts, id);
    ...
    const items = db
        .prepare("select id from checklist_items where event_id = ? and deleted_at is null")
        .all(id);
    for (const it of items)
        await softDeleteChecklistItem(it.id);
}
/**
...
export async function searchEvents(query, limit = 12) {
    ...
    return db
        .prepare(`select * from calendar_events where deleted_at is null
...
export async function listChecklist(eventId) {
    const db = await getDb();
    return db
        .prepare(`select * from checklist_items where deleted_at is null and event_id = ?
       order by coalesce(position, 9999), created_at`)
        .all(eventId);
}
export async function upsertChecklistItem(input, opts = {}) {
    const db = await getDb();
    const ts = nowIso();
    const row = {
    ...
    if (!opts.skipOutbox)
        await enqueue("checklist_items", row.id, "upsert", row);
    return row;
}
...
export async function softDeleteChecklistItem(id) {
    const db = await getDb();
    const ts = nowIso();
    db.prepare("update checklist_items set deleted_at = ?, updated_at = ? where id = ?").run(ts, ts, id);
...
export async function calendarStats() {
    ...
    const todaysTasks = db
        ...
        .get(today.toISOString(), tomorrow.toISOString()).c;
    const upcomingExams = db
        .prepare(`select count(*) as c from calendar_events
         where deleted_at is null and type = 'exam'
           and start_at >= ? and start_at < ?`)
        .get(today.toISOString(), inAWeek.toISOString()).c;
    const tasksCompletedThisWeek = db
        .prepare(`select count(*) as c from calendar_events
         where deleted_at is null and status = 'completed'
           and updated_at >= ?`)
        .get(weekStart.toISOString()).c;
 ...
 */
export async function ensureCalendarBackfill() {
    const db = await getDb();
    const flag = db
        .prepare("select value_json from settings where key = ?")
        .get(BACKFILL_SETTING_KEY);
    if (flag?.value_json === "true")
        return;
    const tasks = db
        .prepare("select id, plan_id, note_id, title, type, scheduled_for, duration_minutes, completed_at, created_at, updated_at from study_tasks where deleted_at is null")
        .all();
    for (const t of tasks) {
        const startMs = new Date(t.scheduled_for).getTime();
        const endIso = new Date(startMs + Math.max(15, t.duration_minutes) * 60_000).toISOString();
        // Map legacy task types onto the richer CalendarEventType enum.
        const type = t.type === "quiz"
            ? "quiz"
            : t.type === "flashcards"
                ? "flashcards"
                : t.type === "read"
    ...
    const ts = nowIso();
    db.prepare(`insert into settings (key, value_json, updated_at) values (?, 'true', ?)
     on conflict(key) do update set value_json='true', updated_at=excluded.updated_at`).run(BACKFILL_SETTING_KEY, ts);
}
