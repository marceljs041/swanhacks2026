...
export function rewardLevel(lifetimeXp: number): RewardLevel {
  const current =
    [...LEVELS].reverse().find((level) => lifetimeXp >= level.min) ?? LEVELS[0];
...
export function loadEquippedUpgradeIds(ownedIds: string[]): string[] {
  ...
  const saved = Array.isArray(parsed)
    ? parsed.filter((id): id is string => typeof id === "string" && owned.has(id))
    : [];
  if (saved.length === 0 && owned.has("cozyScarf")) return ["cozyScarf"];
  return saved;
}

export function saveEquippedUpgradeIds(ids: string[]): void {
...
export function equipUpgrade(
  ...
  const sameSlot = new Set(
    UPGRADE_ITEMS.filter((item) => item.slot === upgrade.slot).map((item) => item.id),
  );
  return [...equippedIds.filter((id) => !sameSlot.has(id)), upgrade.id];
}

export function goatPreviewImage(equippedIds: string[]): string {
  return equippedIds.includes("cozyScarf") ? GOAT_SCARF_IMAGE : GOAT_BASE_IMAGE;
}

export function buildWeeklyChallenge(progressCurrent: number): WeeklyChallenge {
  return {
...
export function buildBadges(
  ...
  const rows: Array<Omit<Badge, "unlocked" | "unlockedAt"> & { profileIds?: string[] }> = [
    {
      id: "welcome_aboard",
      name: "Welcome Aboard",
