...
interface Holiday {
  bucket: Bucket;
  /** True if today is this holiday. */
  match: (d: Date) => boolean;
}

function isNthWeekday(d: Date, month: number, weekday: number, n: number): boolean {
  if (d.getMonth() !== month) return false;
  if (d.getDay() !== weekday) return false;
  return Math.ceil(d.getDate() / 7) === n;
}

function isLastWeekday(d: Date, month: number, weekday: number): boolean {
  if (d.getMonth() !== month) return false;
  if (d.getDay() !== weekday) return false;
...
function timeOfDayBucket(d: Date): Bucket {
  const h = d.getHours();
...
function dayOfWeekBucket(d: Date): Bucket | null {
  const day = d.getDay();
  const h = d.getHours();
  if (day === 5 && h >= 14 && h < 18) return fridayAfternoon;
  if (day === 0 && h >= 17 && h < 22) return sundayEvening;
  if (day === 1 && h >= 6 && h < 11) return mondayMorning;
  return null;
}

function activeHoliday(d: Date): Bucket | null {
  for (const h of holidays) {
    if (h.match(d)) return h.bucket;
  }
  return null;
}

function pickBucket(d: Date): Bucket {
...
export function getGreeting(name: string, now: Date = new Date()): GreetingResult {
