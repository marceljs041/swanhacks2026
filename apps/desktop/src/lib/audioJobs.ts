...
import { chunkAudioFor30s, type AudioChunk } from "./audioChunks.js";
...
export type AudioJobPhase =
  | "preparing"   // decoding + chunking on the renderer
  | "uploading"   // streaming chunks to the sidecar
  | "generating"  // sidecar is running the model (finalize call)
  | "done"        // note has been updated; toast lingers briefly
  | "error"
  | "cancelled";
...
export const useAudioJobs = create<JobsState>((set) => ({
  jobs: {},
  order: [],
  upsert: (id, patch) =>
    set((state) => {
      const prev = state.jobs[id];
      if (!prev && !patch) return state;
      const next: AudioJob = {
        id,
...
export function selectActiveJobForNote(noteId: string): AudioJob | null {
  const all = useAudioJobs.getState().jobs;
  for (const id of Object.keys(all)) {
    const j = all[id];
    if (j && j.noteId === noteId) return j;
  }
  return null;
}
 ...
 */
export function startAudioJob(input: StartJobInput): string {
...
export function cancelAudioJob(id: string): void {
...
export function isTerminal(phase: AudioJobPhase): boolean {
...
async function runJob(
  ...
  const update = (patch: Partial<AudioJob>): void => {
  ...
  let session: AudioSessionCreateResponse;
  try {
    session = await ai.createAudioSession();
  } catch (e) {
    throw new Error(
      "Couldn't reach the local AI sidecar. Is it running? " +
        ((e as Error).message ?? ""),
    );
  }
  if (controller.signal.aborted) return;
  ...
  let result: AudioSessionFinalizeResponse;
  try {
    ...
    throw new Error(
      "Local model failed while generating notes: " + ((e as Error).message ?? ""),
    );
  }
  if (controller.signal.aborted) return;
 ...
 * already IS markdown. */
function composeNoteBody(r: AudioSessionFinalizeResponse): string {
  const parts: string[] = [];
  if (r.summary?.trim()) {
  ...
  if (r.key_terms?.length) {
    ...
    for (const kt of r.key_terms) {
      ...
      if (term) parts.push(`- **${term}** — ${def}`);
    }
  }
  if (r.backend === "stub" && parts.length === 0) {
    parts.push(
      "_Audio captured but the local Gemma 4 audio backend wasn't " +
