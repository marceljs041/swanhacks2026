...
export function headerPadPx(hourHeight: number): number {
...
export function minutesToTop(
  ...
  return (
    ((minutes - dayStartHour * 60) / 60) * hourHeight + headerPadPx(hourHeight)
  );
}

/** Convert a pixel y-offset (within the timed grid) back to minutes-since-midnight. */
export function topToMinutes(
  top: number,
  dayStartHour: number,
  hourHeight: number,
): number {
  return (
    Math.round(((top - headerPadPx(hourHeight)) / hourHeight) * 60) +
    dayStartHour * 60
  );
}
 ...
 */
export function isoFromDayMinutes(dayAnchor: Date, minutes: number): string {
...
export function dayAnchor(iso: string): Date {
  const d = new Date(iso);
  d.setHours(0, 0, 0, 0);
  return d;
}
...
export function fmtMinutesOfDay(minutes: number): string {
